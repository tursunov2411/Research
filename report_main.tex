...
\begin{figure}[H]
  ...
  \caption{Manufacturing Value Added (\% GDP), Uzbekistan 2010--2023: Structural Break at the 2017 Reform Breakpoint. The orange dashed vertical line marks the 2017 reform year. Grey and blue shaded bands represent 95\% confidence intervals for pre- and post-reform linear trends, respectively. The post-reform slope steepens visibly. Chow test: $F = 6.937$, $p = 0.013$. Source: World Bank WDI.}
...
\begin{figure}[H]
  ...
  \caption{Revealed Comparative Advantage Heatmap, Uzbekistan: Top Export Sections by RCA Index for 2015, 2018, 2020, 2022, and 2023. Darker blue indicates higher RCA; Precious Metals (HS~14) dominate with RCA $>$20. Source: Author calculations based on OEC/BACI export data.}
...
\begin{figure}[H]
  ...
  \caption{Manufacturing-Related RCA Trends for Five Sectors (Instruments, Machines, Metals, Textiles, Transportation), Uzbekistan, 2010--2023. The horizontal dashed red line marks the RCA~=~1 threshold. Notable features: Transportation spikes near 2012 (GM Uzbekistan), collapses, then rebounds post-2021 (Hyundai); Textiles erode toward 1; Machines remain persistently below~1. Source: Author calculations based on OEC/BACI export data.}
...
\begin{figure}[H]
  ...
  \caption{Uzbekistan Bilateral Trade with China, Japan, and South Korea, 2010--2023. Panel~A: total trade (exports~+~imports, USD~billion); Panel~B: trade balance (exports minus imports). Orange vertical dashed line marks the 2017 reform. China's trade dominates in both scale and deficit. Source: OEC/BACI public API.}
...
\begin{figure}[H]
  ...
  \caption{Share of Intermediate Goods in Uzbekistan's Imports from China, Japan, and South Korea, 2010--2023. Intermediate goods proxied by HS sections 07, 15, 16, 17, and 18. Orange vertical dashed line marks the 2017 reform. Japan's share approaches 93--100\%; South Korea 87--92\%; China rises from $\sim$70\% to $\sim$85\%. Source: OEC/BACI public API.}
...
\begin{figure}[H]
  ...
  \caption{Top Investor-Country Shares in Uzbekistan's Foreign Investment and Loans, 2024. China leads at $\sim$28\%; Russia $\sim$12\%; Turkey $\sim$8\%. Critically, Japan and South Korea do not appear among the top eight investors, despite growing bilateral trade. Source: National Statistics Committee of Uzbekistan.}
...
\begin{figure}[H]
  ...
  \caption{Uzbekistan's Directed Trade Network and Value-Chain Positioning. Nodes: UZB (orange, hub), CHN/JPN/KOR (teal, NE~Asian sources), KAZ/RUS/TUR (green, re-export markets), ROW (purple, commodity destination). Edge colours: orange = intermediate imports, blue = commodity exports, green = consumer/textile exports, grey = re-export potential. Edge width $\propto \log(\text{trade volume})$; opacity $\propto$ intermediate goods share. Note: network is constructed from bilateral merchandise trade flows as a proxy; Uzbekistan is not covered by the OECD TiVA database. Source: Author calculations from OEC/BACI HS-section bilateral trade data.}
...
\begin{figure}[H]
  ...
  \caption{Intermediate Goods Dependence: Uzbekistan's Imports from Northeast Asia, 2010--2023. Panel~A: intermediate goods share (\%) --- note that partner-level shares appear near-constant because the HS-section proxy captures broad categories that remain structurally stable; the meaningful variation is in absolute volumes (Panel~B). Panel~B: absolute intermediate import volume (USD~bn), showing China's post-2017 surge. BEC Rev.5 proxy: HS sections 05, 06, 07, 15, 16, 17, 18. Orange dashed line marks the 2017 reform. Source: Author calculations from OEC/BACI bilateral trade data.}
...
\begin{figure}[H]
  ...
  \caption{Uzbekistan Export Composition by Category, 2010--2023. Panel~A: category shares (\%); Panel~B: absolute volumes (USD~bn). Six categories shown: Gold/Precious Metals (HS~14, dominant at 50--60\%), Textiles, Other Primary, Intermediate Input, Capital/Machines, and Consumer/Other. Manufactured and machinery exports (HS~15--18 combined) remain below 10\% in aggregate. White dashed line marks the 2017 reform. Source: Author calculations from OEC/BACI HS-section export data.}
...
\begin{figure}[H]
  ...
  \caption{Re-export Potential: Uzbekistan's Manufactured Goods Exports to Eurasian Markets (2023). Tile chart showing trade flow volumes (USD~billion) by destination and flow type; colour intensity $\propto$ trade value. Kazakhstan represents the largest near-term corridor. All flows remain below \$1~billion. Source: OEC/BACI bilateral trade data.}
...
\begin{figure}[H]
  ...
  \caption{GVC Positioning: Uzbekistan's Backward vs.\ Forward Integration, 2010--2023. The ideal trajectory is toward the upper-right quadrant; the \textit{actual} trajectory moves rightward (increasing backward integration) but stagnates or declines vertically (forward integration), confirming captive governance. Backward proxy = intermediate goods share of imports from NE~Asia; Forward proxy = manufactured goods share of total exports. Colour: grey = pre-reform, orange = early reform (2017--2020), teal = post-COVID (2021--2023). Source: Author calculations from OEC/BACI and World Bank WDI data.}
...
\begin{figure}[H]
  ...
  \caption{Uzbekistan versus Vietnam: Comparative Reform Trajectory. Three panels: (A)~Manufacturing VA (\%~GDP), (B)~FDI inflows (\%~GDP), (C)~Logistics Performance Index. X-axis = years since major liberalisation (year~0 = 2017 for Uzbekistan, 2007 WTO accession for Vietnam). The FDI gap (Panel~B) is the most dramatic divergence. Source: World Bank WDI.}
...
\begin{figure}[H]
  ...
  \caption{Uzbekistan Manufacturing Value Added Forecast, 2010--2030. Three scenarios diverge from 2024: Scenario~A (Status Quo, linear extrapolation), Scenario~B (Accelerated GVC Embedding), Scenario~C (Vietnam Trajectory). Grey shaded band = 80\% baseline forecast interval. Orange dotted vertical line marks the 2017 reform; solid line marks the historical/forecast boundary at 2024. Source: World Bank WDI and author calculations.}
  \label{fig:forecast}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Trade Openness (normalised index, 2010~=~100), Manufacturing VA (\%~GDP), and ECI Trajectory, Uzbekistan 2010--2023. Dot size and colour reflect ECI level (red $\approx$ --0.6, yellow-green $\approx$ --0.2); the path traces annual movement. The post-2017 trajectory shows simultaneous improvement in openness, manufacturing, and complexity, plateauing circa 2021. Source: World Bank WDI and OEC.}
  \label{fig:mechanism}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Key Indicator Trends, Uzbekistan, 2010--2023. Three panels: (A)~FDI Inflows (\%~GDP, with dual-scale axis), (B)~GDP per Capita (current USD), (C)~Manufacturing VA (\%~GDP). Source: World Bank WDI.}
...
\begin{figure}[H]
  ...
  \caption{Uzbekistan Scenario Dashboard, 2010--2030: Manufacturing VA, FDI Inflows, and Economic Complexity Index under three policy scenarios (A: Status Quo, B: Accelerated GVC, C: Vietnam Trajectory). See Figure~\ref{fig:forecast} for detailed scenario definitions. Source: World Bank WDI, OEC, and author calculations.}
